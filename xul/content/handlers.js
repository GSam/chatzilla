...
function failWaitingResumes (user)
{
    if (! user.resumeAttempts)
        return false;

    var numResumes = 0, ary;
    for (ary in user.resumeAttempts)
    {
        var info = user.resumeAttempts[ary];
        clearTimeout(info.ct);

        // Count the number of resume attempts noted
        numResumes++;

        // Only activate the fallback on DCC FILE transfers that are
        // still waiting for a response to their DCC RESUME requests
        if (info.o.state.state == DCC_STATE_RESUMING)
            setTimeout(info.fallback, 0, info.o, info.folder);
    }

    delete user.resumeAttempts;

    return (numResumes > 0);
}

CIRCNetwork.prototype.on486 =
function my_registrationRequired (e)
{
    var ary;

    if (/^You must identify to a registered nick/.test(e.params[3]))
    {
        e.user = new CIRCUser(e.server, null, e.params[2]);

        if (failWaitingResumes(e.user))
        {
            // FIXME: Should we record that this user has the +R usermode set,
            //        so the DCC Resume option will no longer be offered for
            //        DCC File transfers with this user?
            //        (If so, we should watch for our getting a registered
            //         nickname.)

            display(getMsg(MSG_DCCFILE_ERR_NOREGISTER, e.user.unicodeName),
                    "DCC-FILE");

            // We don't need to run the default handler if we believe this
            // message was sent as a response to our DCC RESUME request(s)
            return;
        }
    }
...
function my_noCTCP (e)
{
    ...
    if (ary = e.params[2].match(/^(\S+) does not accept CTCPs$/i))
    {
        ...
        if (failWaitingResumes(e.user))
        {
            ...
            //        (i.e. the user's +T usermode is set),
            //        so the DCC Resume option will no longer be offered for
            //        DCC File transfers with this user?

            display(getMsg(MSG_DCCFILE_ERR_NOCTCP, e.user.unicodeName),
                    "DCC-FILE");
...
{
    // When making a DCC RESUME request, we use PING as a convenient,
    // well-handled CTCP message that everyone responds to -- i.e. nobody ever
    // sends ERRMSG CTCP messages in response to an unknown DCC message, and
    // we don't want to rely on timeouts exclusively (as the actual timeout
    // needed will depend on the actual round-trip time to get a response, and
    // if the offering user got and properly processed our DCC RESUME request
    // and we don't realize it did, our normal DCC FILE transfer WILL fail), so
    // we send a PING message and assume that the other client doesn't
    // understand DCC RESUME if we don't get a reply "quickly" after the PING
    // response (since some clients might prioritize sending PING responses
    // over other transmissions).

    // Unfortunately, not all IRC clients respond to arbitrary strings PINGed.
    // Therefore, this simply checks if the time attached is one sent during a
    // resume request to determine if the PING response was user-generated or
    // generated by the DCC RESUME request generator.
    if (e.user.resumeAttempts && e.user.resumeAttempts[e.CTCPData])
    {
        var info = e.user.resumeAttempts[e.CTCPData];

        // Disable the "final fallback" timeout
        clearTimeout(info.ct);

        // If we're still waiting for a response, set the fallback to trigger
        if (info.o.state.state == DCC_STATE_RESUMING)
        {
            setTimeout(info.fallback,
                       client.prefs["dcc.resume.fallback.delay"],
                       info.o, info.folder);
        }

        delete e.user.resumeAttempts[e.CTCPData];
        return;
    }
...
function findAcceptableAutoFilename(o, checkFunction)
{
    ...
    while (true)
    {
        ...
        if (++tries > 1)
        {
            ...
            leaf = leaf.replace(/(\.[a-z]*\.(gz|bz2|[7x]?z|lzma)|\.[^\.]*|)$/i,
                                "[" + tries + "]$&");
        }

        dest = getFileFromURLSpec(o.folder);
        dest.append(leaf);
        leaf = checkFunction(dest);
        if (leaf == 1)
            return dest;
        if (leaf == -1)
            return "";
...
function onDCCAutoAcceptTimeout(o, selection)
{
    ...
    if (o.suppressAuto)
    {
        // If we are suppressing auto-actions (because a file picker is active
        // for the file transfer in question), put off this action for another
        // second
        setTimeout(onDCCAutoAcceptTimeout, 1000, o, selection);
        return;
    }

    function _forAccept (dest)
    {
        return (dest.exists() ? 0 : 1);
    }

    var dest = findAcceptableAutoFilename(selection, _forAccept);
    o.accept(dest);
    display(getMsg(MSG_DCCFILE_ACCEPTED, o._getParams()), "DCC-FILE");
}

function setupResumeFallback(fb)
{
    function timeoutFallback(u, id)
    {
        // If we never receive a PING response, we still need
        // clear out our test ID and run our fallback.
        var info = u.resumeAttempts[id];
        if (info.o.state.state == DCC_STATE_RESUMING)
            setTimeout(info.fallback, 0, info.o, info.selection);
        delete u.resumeAttempts[id];
    }

    // If the container for resume attempts for this user doesn't exist yet,
    // create it.
    if (! fb.o.user.netUser.resumeAttempts)
        fb.o.user.netUser.resumeAttempts = {};

    // Get a new (presumably unique) ID for our DCC RESUME attempt, store the
    // passed data for our eventual fallback, set a timeout to backstop the
    // PING response check, and send the PING to the client offering the file
    // transfer we are trying to resume for.
    var resumeTestID = Number(new Date());
    fb.o.user.netUser.resumeAttempts[resumeTestID] = fb;
    fb.ct = setTimeout(timeoutFallback,
                       client.prefs["dcc.resume.fallback.maxDelay"],
                       fb.o.user.netUser, resumeTestID);
    fb.o.user.netUser.ctcp("PING", resumeTestID);
}

function onDCCCmdResumeTimeout(o, data)
{
    // This is the fallback used when DCC RESUME is attempted using the
    // /dcc-resume command (not the automatic resume).

    // It's possible for this fallback to be called when, in fact, the resume
    // attempt succeeded, so we check for that possibility.
    if (o.state.state != DCC_STATE_RESUMING)
        return;

    // Present a message to the user about our failure to resume, and present
    // the choice of accepting the file transfer from the beginning.
    // Ensure there are no 8) faces in the message where we fill in numbers!
    client.munger.getRule(".inline-buttons").enabled = true;
    client.munger.getRule("face").enabled = false;
    display(getMsg(MSG_DCCFILE_ERR_RESUME_FAIL,
                   [o.user.unicodeName, o.filename, o.port,
                    getSISize(o.resumingPick.fileSize), getSISize(o.size),
                    getMsg(MSG_DCC_COMMAND_ACCEPTC, "dcc-accept " + o.id)
                   ]),
            "DCC-FILE");
...
function onDCCAutoResumeTimeout(o, selection)
{
    // user may have already accepted or declined
    if (o.state.state != DCC_STATE_REQUESTED)
        return;
    if (o.suppressAuto)
    {
        ...
        // for the file transfer in question), put off this action for another
        // second
        setTimeout(onDCCAutoResumeTimeout, 1000, o, selection);
        return;
    }

    function _forAutoResume (chk)
    {
        if (!chk.exists())
            return -1;
        if (!chk.isFile() || !chk.isReadable() || !chk.isWritable() ||
            chk.fileSize >= o.size)
        {
            return 0;
        }

        // Do not try to resume into files that we _know_ are currently
        // receiving data from another active file transfer.
        if (client.dcc.findFileTransfer(chk, o))
            return 0;

        return 1;
    }

    var dest = findAcceptableAutoFilename(selection, _forAutoResume);

    // If there are no valid targets for auto-resume in the downloads folder
    // (because the user renamed, moved, or deleted the targets previously
    // noted), fall back on transferring from scratch.
    if (dest == "")
        return onDCCAutoAcceptTimeout(o, selection);

    if (o.resume(dest))
    {
        display(getMsg(MSG_DCCFILE_RESUMING_CMD,
                       o._getParams().concat([getSISize(dest.fileSize)])),
                "DCC-FILE");

        setupResumeFallback({o: o, fallback: onDCCAutoAcceptTimeout,
                             selection: selection });
    }
    else
    {
        onDCCAutoAcceptTimeout(o, selection);
    }
}

function onDCCReofferResumeTimeout(o, data)
{
    // This is the fallback used when DCC RESUME is attempted in a reoffering
    // catch.
    ...
    client.munger.getRule(".inline-buttons").enabled = true;
    client.munger.getRule("face").enabled = false;
    display(getMsg(MSG_DCCFILE_REOFFER_NO_RESUME,
                   [o.user.unicodeName, o.filename, getSISize(o.size), o.port,
                    getMsg(MSG_DCC_COMMAND_ACCEPTC, "dcc-accept " + o.id) +
                    " " +
                    getMsg(MSG_DCC_COMMAND_DECLINE, "dcc-decline " + o.id)
                   ]),
            "DCC-FILE");
    client.munger.getRule("face").enabled = client.prefs["munger.face"];
    client.munger.getRule(".inline-buttons").enabled = false;

    // ... If this is, indeed, a reoffering, a 'decline' should probably
    //     be sent to prevent further automatic reoffering of the same file ...
}

function onDCCReofferTimeout(transfer, o, file)
{
    // user may have already accepted or declined, or it may already be attached
    if ((o.state.state != DCC_STATE_REQUESTED) || (o.reofferFor))
        return;

    if (o.suppressAuto)
    {
        // If we are suppressing auto-actions (because a file picker is active
        // for the new file transfer in question), put off this action for
        // another second
        setTimeout(onDCCReofferTimeout, 1000, transfer, o, file);
        return;
    }

    // Attach the reoffer; setReoffer should present any needed error messages
    if (! o.setReoffer(transfer, file))
        return;

    // Tell the user that the reoffer has been attached
    client.munger.getRule("face").enabled = false;
    display(getMsg(MSG_DCCFILE_REOFFER_ATTACHED,
                   [o.filename, getSISize(o.size), o.user.unicodeName,
                    o.remoteIP, o.port, transfer.localFile.localFile.leafName,
                    transfer.port]),
            "DCC-FILE");
    client.munger.getRule("face").enabled = client.prefs["munger.face"];
}

function onDCCDeclineIgnored(o)
{
    o.decline();
}

CIRCUser.prototype.onDCCChatHook =
CIRCUser.prototype.onDCCSendHook =
function my_dcchook_final(e)
{
    if (e.ignore || e.autoAccept)
        return;

    // If no extension already set e.autoAccept, we should check if the
    // offering user is on our auto-accept list
    var allowList = this.parent.parent.prefs["dcc.autoAccept.list"];
    for (var m = 0; m < allowList.length; ++m)
    {
        if (hostmaskMatches(e.sourceEvent.user,
                            getHostmaskParts(allowList[m]), this.parent))
        {
            e.autoAccept = true;
            return;
        }
...
function my_dccchat(e)
{
    ...
    var ev = new CEvent("user", "dcc-chat-hook", this, "onDCCChatHook");
    ev.sourceEvent = e;
    ev.autoAccept = false;
    ev.ignore = false;
    ev.delay = client.prefs["dcc.autoAccept.delay"];
    ev.dccChat = c;
    ...
    // Provide a convenient 'hook' for an extension used to auto-accept
    // 'expected' chats (Fserv interfaces, etc.)
    // Delays, acceptance, etc. should be marked in the 'ev' object passed.
    client.eventPump.routeEvent(ev);


    // Pass the event over to the DCC Chat object when we exit this function
    e.set = "dcc-chat";
    e.destObject = c;
    e.destMethod = "onGotRequest";


    // Check if the hook wants to ignore this event; we still need to run
    // onGotRequest to finalize the DCC Chat object enough to decline it.
    if (ev.ignore)
    {
        setTimeout(onDCCDeclineIgnored, 0, c);
        return;
    }
    ...
    if (ev.autoAccept)
    {
        ...
        else
        {
            ...
            cmds = [(ev.acceptDelay / 1000), cmds];
        }
        setTimeout(onDCCAutoAcceptTimeout, ev.acceptDelay, c);
    }
...
function my_dccsend(e)
{
    ...
    var ev = new CEvent("user", "dcc-send-hook", this, "onDCCSendHook");
    ev.sourceEvent = e;
    ev.autoAccept = false;
    ev.resume = false;
    ev.ignore = false;
    ev.folder = this.parent.parent.prefs["dcc.downloadsFolder"];
    ev.filename = f.filename;
    ev.delay = client.prefs["dcc.autoAccept.delay"];
    ev.dccFileTransfer = f;

    var str = MSG_DCCFILE_GOT_REQUEST;
    var cmds = getMsg(MSG_DCC_COMMAND_ACCEPT, "dcc-accept " + f.id) + " ";


    // If the offering client doesn't offer a transfer size, it almost
    // certainly does not support DCC RESUME.
    if (e.size > 0)
    {
        ev.resume = true;

        // FIXME: Does the other person accept CTCP messages?
        //        If not, we can't resume, and we shouldn't show the option
        //  ... can we check this 'ahead of time' ?
        //  ... what about users that don't do resume anyway?
        //      can we cache this?  should we cache this?

        // If we believe DCC RESUME may be available, we should include that
        // command among those presented to our user.
        if (ev.resume)
        {
            cmds += getMsg(MSG_DCC_COMMAND_RESUME, "dcc-resume " + f.id) + " ";
            ev.resume = { allow: client.prefs["dcc.autoResume"] };
        }
    }

    cmds += getMsg(MSG_DCC_COMMAND_DECLINE, "dcc-decline " + f.id);


    // Provide a convenient 'hook' for an extension used to auto-accept
    // 'expected' transfers into pre-selected folders (Fserv interfaces, etc.)
    // Folders, delays, etc. should be marked in the 'ev' object passed.
    client.eventPump.routeEvent(ev);


    // Pass the event over to the DCC File object when we exit this function
    e.set = "dcc-file";
    e.destObject = f;
    e.destMethod = "onGotRequest";
    ...
    // Check if the hook wants to ignore this event; we still need to run
    // onGotRequest to finalize the DCC File object enough to decline it.
    if (ev.ignore)
    {
        setTimeout(onDCCDeclineIgnored, 0, f);
        return;
    }

    delete ev.currentTransfer;


    // If we believe DCC Resume is an option, and we are allowed to auto-resume
    // transfers, we should check if there are any files in the downloads
    // folder which might be a valid target to resume with this transfer.
    if (ev.resume && ev.resume.allow && ev.autoAccept)
    {
        function _forResume (chk)
        {
             if (!chk.exists())
                 return -1;
             if (chk.isFile() && chk.isReadable() && chk.isWritable() &&
                 chk.fileSize < e.size)
             {
                 var eft = client.dcc.findFileTransfer(chk, f);
                 if (! eft)
                     return 1;
                 if (eft.isSameFile(f))
                 {
                     // This file is already receiving a file transfer from
                     // the same user with the same size and name...

                     // The user might want to cancel the transfer, and we
                     // should wait at least a second to check if the
                     // previous file transfer is gone.
                     if (ev.delay < 1000)
                         ev.delay = 1000;

                     ev.currentTransfer = eft;
                     return 1;
                 }
             }
             return 0;
        ...
        var dest = findAcceptableAutoFilename({folder: ev.folder,
                                               filename: ev.filename},
                                              _forResume);
        if (dest == "")
            ev.resume = false;
        else if (ev.currentTransfer)
            ev.resume = dest;

        // Remember what file name and how far done it is, so this information
        // can be presented to the user
        else if (ev.delay > 0)
            ev.resume = {name: dest.leafName, place: getSISize(dest.fileSize) };
    }
    else
    {
        ev.resume = false;
    ...
    if (ev.currentTransfer)
    {
        str = MSG_DCCFILE_RESUMING_ACTIVE;
        cmds = getMsg(MSG_DCC_COMMAND_STOP_CURRENT,
                      "dcc-stop-n-resume " + ev.currentTransfer.id +
                      " " + f.id) + " " + cmds;
        f.lastFile = ev.resume;

        // Don't use a timeout if the existing transfer already has a command-
        // assigned reoffer
        if (! ev.currentTransfer.reofferByCmd)
        {
            cmds = getMsg(MSG_DCCFILE_RESUMING_REAUTO,
                          [(ev.delay / 1000), ev.resume.leafName]) + " " + cmds;
            setTimeout(onDCCReofferTimeout,
                       ev.delay, ev.currentTransfer, f, ev.resume);
        }
    }
    else if (ev.autoAccept)
    {
        if (ev.delay == 0)
        {
            str = ev.resume ? MSG_DCCFILE_RESUMING_NOW
                            : MSG_DCCFILE_ACCEPTING_NOW;
        }
        else if (ev.resume)
        {
            str = MSG_DCCFILE_RESUMING;
            cmds = [(ev.delay / 1000),
                    ev.resume.name, ev.resume.place, cmds];
        }
        else
        {
            str = MSG_DCCFILE_ACCEPTING;
            cmds = [(ev.delay / 1000), cmds];
        }
        setTimeout(ev.resume ? onDCCAutoResumeTimeout
                             : onDCCAutoAcceptTimeout,
                   ev.delay, f, {folder: ev.folder, filename: ev.filename});
    }
...
CIRCUser.prototype.onDCCResume =
function my_dccresume(e)
{
    // This is called when someone else sends a DCC RESUME request to our
    // client.  It identifies the affected file transfer offer (if any) and
    // passes the request on to the relevant DCC file handler for processing
    ...
    var list = client.dcc.getMatches
              (u.unicodeName, e.file, ["file"], [DCC_DIR_SENDING],
               [DCC_STATE_REQUESTED], e.port);

    // If there is not exactly one match, there is no active request to act on.
    if (list.length != 1)
        return;
    ...
    e.set = "dcc-file";
    e.destObject = list[0];
    e.destMethod = "onGotResume";
}

CIRCUser.prototype.onDCCAccept =
function my_dccaccept(e)
{
    // This is called when our DCC RESUME request to another client results in
    // a successful response to us.  It identifies the affected file transfer
    // offer and confirms the resuming transfer, and presents the "resuming in
    // progress" success message to the user
    ...
    var u = client.dcc.addUser(e.user, e.host);
    var list = client.dcc.getMatches
              (u.unicodeName, e.file, ["file"], [DCC_DIR_GETTING],
               [DCC_STATE_RESUMING], e.port);

    // If there's 0 matches, and the filename contains spaces, there ARE
    // clients that present spaces in their ACCEPT message and underscores
    // in their SEND message... :(
    if ((list.length == 0) && (/\s/.test(e.file)))
    {
        list = client.dcc.getMatches
              (u.unicodeName, e.file.replace(/\s/g, "_"),
               ["file"], [DCC_DIR_GETTING], [DCC_STATE_RESUMING], e.port);
    }
    ...
    if (! list[0].resuming(e.resumeAt))
        return;

    // Notify the user that the file transfer is resuming successfully
    this.parent.parent.display
        (getMsg(MSG_DCCFILE_RESUMED,
                list[0]._getParams().concat([getSISize(e.resumeAt)])),
                "DCC-FILE");
}
...
{
    var ip = this.state.dir == DCC_DIR_GETTING ? this.remoteIP : this.localIP;
    return [this.unicodeName, ip, this.port];
}
...
function my_dccfilegetparams()
{
    ...
    var ip = this.state.dir == DCC_DIR_GETTING ? this.remoteIP : this.localIP;
    return [this.filename, dir, this.unicodeName, ip, this.port];
}

CIRCDCCFileTransfer.prototype.onConnect =
function my_dccfileconnect(e)
{
    // FIXME: If this.reofferFor exists, we might prefer to reuse the existing
    //        tab, if possible
    ...
    // Remove unneeded properties (used in reoffer handling)
...
function dccfile_reoffer_off(trs, reoffer, msg)
{
    // Present one of the "Reoffer unavailable" messages to the user
    var cmds;
    cmds = getMsg(MSG_DCC_COMMAND_ACCEPT, "dcc-accept " + reoffer.id) + " " +
           getMsg(MSG_DCC_COMMAND_RESUME, "dcc-resume " + reoffer.id) + " " +
           getMsg(MSG_DCC_COMMAND_DECLINE, "dcc-decline " + reoffer.id);

    client.munger.getRule(".inline-buttons").enabled = true;
    client.munger.getRule("face").enabled = false;
    display(getMsg(msg,
                   [trs.user.unicodeName, trs.remoteIP, trs.port,
                    trs.filename, getSISize(trs.size), reoffer.port, cmds]),
            "DCC-FILE");
    client.munger.getRule("face").enabled = client.prefs["munger.face"];
    client.munger.getRule(".inline-buttons").enabled = false;
}

CIRCDCCFileTransfer.prototype.useReoffer =
function my_dccfile_use_reoffer()
{
    // On SeaMonkey 1.1.14, this next line is required to update the
    // nsLocalFile instance's file status information.
    var file = new nsLocalFile(this.lastFile.path);

    // Use our recorded 'file' to actually start the resume attempt,
    // and set up the fallback for possible failure
    if (this.resume(file))
    {
        display(getMsg(MSG_DCCFILE_RESUMING_CMD,
                       this._getParams().concat([getSISize(file.fileSize)])),
                "DCC-FILE");

        setupResumeFallback({o: this, fallback: onDCCReofferResumeTimeout });
        return true;
    }

    // If resume couldn't even be started, the offer probably no longer exists
    delete this.lastFile;
    return false;
}

CIRCDCCFileTransfer.prototype.setReoffer =
function my_dccfile_set_reoffer(oldTransfer, file, isByCommand)
{
    if ((file || oldTransfer) && (this.state.state != DCC_STATE_REQUESTED))
    {
        // Complain if the prospective transfer isn't prospective any more
        display(MSG_DCCFILE_ERR_NO_OFFER, "DCC-FILE");
        return (! isByCommand);
    }

    if (oldTransfer)
    {
        if (oldTransfer.state.state == DCC_STATE_DONE)
        {
            // If the previous transfer actually managed to finish, then a
            // reoffer can't continue it... and it can be questioned if it's
            // actually a reoffer at all!
            dccfile_reoffer_off(oldTransfer, this, MSG_DCCFILE_REOFFER_DONE);
            return (! isByCommand);
        }

        if (! isByCommand)
        {
            if (oldTransfer.state.state == DCC_STATE_ABORTED)
            {
                // If the user aborted the previous transfer and didn't
                // specifically command the reorder's use, don't use it
                dccfile_reoffer_off(oldTransfer, this,
                                    MSG_DCCFILE_REOFFER_ABORT);
                return false;
            }

            if (oldTransfer.isActive() && oldTransfer.reofferByCmd)
            {
                // Reoffers set by command override automatic reoffer assignment
                dccfile_reoffer_off(oldTransfer, this,
                                    MSG_DCCFILE_REOFFER_NOT_AUTO);
                return false;
            }
        }

        if (((oldTransfer.state.state == DCC_STATE_ABORTED) ||
             (oldTransfer.state.state == DCC_STATE_FAILED)) &&
            (oldTransfer.reoffer))
        {
            // If there's an activated reoffer, we should switch to it
            return this.setReoffer(oldTransfer.reoffer, file, isByCommand);
        }

        if (oldTransfer.isActive())
        {
            // If the previous transfer is still active, we can attach the
            // selected reoffer to it
            this.lastFile = file;
            if (this.reofferFor)
                delete this.reofferFor.reoffer;
            this.reofferFor = oldTransfer;
            if (oldTransfer.reoffer)
            {
                // If another offer is already associated with the transfer,
                // disassociate it and inform the user
                var last = oldTransfer.reoffer;
                var cmds = getMsg(MSG_DCC_COMMAND_STOP_CURRENT,
                                  "dcc-stop-n-resume " + this.id + " " +
                                  last.id) + " " +
                           getMsg(MSG_DCC_COMMAND_ACCEPT,
                                  "dcc-accept " + last.id) + " " +
                           getMsg(MSG_DCC_COMMAND_RESUME,
                                  "dcc-resume " + last.id) + " " +
                           getMsg(MSG_DCC_COMMAND_DECLINE,
                                  "dcc-decline " + last.id);

                client.munger.getRule(".inline-buttons").enabled = true;
                client.munger.getRule("face").enabled = false;
                display(getMsg(MSG_DCCFILE_REOFFER_BUMP,
                               [last.user.unicodeName, last.remoteIP, last.port,
                                last.filename, getSISize(last.size), cmds]),
                        "DCC-FILE");
                client.munger.getRule("face").enabled =
                    client.prefs["munger.face"];
                client.munger.getRule(".inline-buttons").enabled = false;

                delete last.reofferFor;
            }
            oldTransfer.reoffer = this;
            if (isByCommand)
                oldTransfer.reofferByCmd = true;
            return true;
        }
    }
    else if (! file)
    {
        // Clear associations between this offer and previous transfers
        if (this.reofferFor)
        {
            delete this.reofferFor.reoffer;
            delete this.reofferFor.reofferByCmd;
            delete this.reofferFor;
            delete this.lastFile;
        }
        return true;
    ...
    // Search for any other transfers associated with the given file
    var transfer = client.dcc.findFileTransfer(file, this);
    if (transfer && (transfer != oldTransfer))
    {
        if (transfer.isSameFile(this))
            return this.setReoffer(transfer, file, isByCommand);

        // The chosen file is already being resumed from a different source,
        // and may not be the same file at all.
        dccfile_reoffer_off(oldTransfer, this, MSG_DCCFILE_REOFFER_MISMATCH);
        return false;
    }

    // If there's no active transfers on the file, and no links to previous
    // transfers on it, try to resume on it using the "useReoffer" function
    this.lastFile = file;
    if (oldTransfer)
    {
        this.reofferFor = oldTransfer;
    ...
    return this.useReoffer();
}

CIRCDCCFileTransfer.prototype.onNoDeviceSpace =
function my_dccfile_fs_full(e)
{
    this.display(getMsg(MSG_DCCFILE_ERR_OUT_OF_SPACE), MT_ERROR);
...
function my_dccfileabort(e)
{
    ...
    if (this.reoffer)
    {
        dccfile_reoffer_off(this, this.reoffer, MSG_DCCFILE_REOFFER_ABORT);
        delete this.reoffer.lastFile;
        delete this.reoffer.reofferFor;
        delete this.reofferByCmd;
    }
    else if (this.reofferFor)
    {
        this.setReoffer();
    }
}
...
function my_dccfiledisconnect(e)
{
    ...
    if (this.state.dir == DCC_DIR_GETTING)
    {
        ...
        if (this.reoffer)
        {
            dccfile_reoffer_off(this, this.reoffer, MSG_DCCFILE_REOFFER_DONE);
            delete this.reoffer.lastFile;
            delete this.reoffer.reofferFor;
            delete this.reoffer;
            delete this.reofferByCmd;
